...
export default function Billing() {
  ...
  const [stage, setStage] = useState('idle'); // idle | submitting | waiting | success | error
        ...
        {PLAN_ORDER.map((key) => {
          ...
          return (
            <div key={key} className={`glass-panel rounded-2xl p-6 flex flex-col ${isCurrent ? 'ring-2 ring-gold-500' : ''}`}>
              ...
              >
                {isCurrent ? 'Active' : `Pay K${plan.price} via Mobile Money`}
              </button>
            </div>
          );
        })}
              ...
              <form onSubmit={submitPayment} className="space-y-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="btn-ghost flex-1 py-3 rounded-xl font-bold text-sm"
                  >
                    Cancel
                  </button>
