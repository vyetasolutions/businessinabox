...
The repo is a **monorepo** with two folders:
```
vyeta-business-hub/
...
5. Go to **Project Settings → API**. Copy three values — you'll need them shortly:
   - `Project URL`
   - `anon public` key
...
## PART 3 — Twilio WhatsApp Sandbox (free)
...
> **Free-tier limitation:** the Sandbox only sends WhatsApp messages to numbers that have joined it, and messages are prefixed with a Twilio sandbox notice. This is fine for testing and small pilots. To message customers who haven't joined the sandbox, you'd eventually need Twilio's paid **WhatsApp Business API** approval — out of scope for this free build, but the backend code doesn't need to change when you upgrade, just the credentials.
...
| `BACKEND_SHARED_SECRET` | any long random string you make up |
...
> **Free-tier note:** Render's free Web Services "spin down" after ~15 minutes of no traffic, and the next request takes 30–60 seconds to "wake up." This mainly affects the WhatsApp webhook and Add Employee button — the first call after inactivity will feel slow. If this matters to you, set up a free external pinger (e.g. UptimeRobot, cron-job.org) to hit `https://your-backend.onrender.com/health` every 10 minutes to keep it warm.

### Wire up the WhatsApp webhook

1. In Supabase, go to **Database → Webhooks → Create a new webhook**.
2. Configure:
   - **Table:** `whatsapp_messages_queue`
   - **Events:** `Insert`
   - **Type:** HTTP Request
   - **URL:** `https://your-backend.onrender.com/api/trigger-whatsapp`
   - **HTTP Headers:** add `x-webhook-secret` = the same value you set for `BACKEND_SHARED_SECRET`
3. Save. Now, every time a document is created, the database trigger already built into `supabase_schema.sql` inserts a row into `whatsapp_messages_queue`, Supabase's webhook fires, and your backend sends the WhatsApp message via Twilio.
...
1. Open your frontend URL and go to `/signup` to create a real test business (this exercises the actual path your future customers will use). Submit it, then check you land on the "pending approval" success screen.
...
3. Go to **Stock** and add a few inventory items.
4. Go to **Dashboard → Add Employee** and create an Employee login. Log out, log back in as that Employee — you should land on the **Sell** (POS) screen only, with Dashboard/Settings/Create Doc hidden.
5. From the POS screen, add stock items to a sale, fill in customer details, and generate a Receipt — a PDF is created in your browser, uploaded to Supabase Storage, and a WhatsApp share link is prepared.
6. Turn off your device's WiFi/data, repeat step 5 — the document still saves (to `localStorage`), shows a "queued for sync" badge, and will automatically push to Supabase the moment you're back online (this is the offline-first PWA behavior — you can also literally install the app to your home screen via the browser's "Add to Home Screen" / install prompt).
...
- **Twilio Sandbox**: free but numbers must join the sandbox and messages carry a sandbox notice; fine for internal use/piloting.
